...
export KUBE_DEPLOYMENT_NAME=alfresco-adf-demo
...
sudo $docker build -t triall.com/alfresco/adf-demo:latest .
sudo $docker tag triall.com/alfresco/adf-demo:latest $REGISTRY_URL/alfresco/adf-demo
...
kubectl -n $KUBE_NAMESPACE expose deployment alfresco-factom-demo --port=80 --name=alfresco-adf-demo
kubectl -n $KUBE_NAMESPACE label service alfresco-factom-demo app=alfresco-adf-demo
